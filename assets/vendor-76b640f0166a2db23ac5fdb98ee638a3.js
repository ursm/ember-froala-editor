...
t.DEFAULTS={initOnClick:!1,pluginsEnabled:null},t.MODULES={},t.PLUGINS={},t.VERSION="2.9.1",t.INSTANCES=[],t.OPTS_MAPPING={},t.SHARED={},t.ID=0,t.prototype._init=function(){var t=this.$oel.prop("tagName")
...
if(E===a(i(a("mcVRDoB1BGILD7YFe1BTXBA7B6==")))||0<=E.indexOf(r,E.length-r.length)||h(r)){if(!((null===(u=b[1])||new Date(u)<new Date(a("uB2C1D7B5E1E5E4A1E3C11==")))&&0<(r||"").length)||h(r)){t.ul=!1
...
var s=""
if("options"!==r.type)if(o){var a="function"==typeof r.defaultSelection?r.defaultSelection(t):r.defaultSelection
s='<span style="width:'+(r.displaySelectionWidth||100)+'px">'+t.language.translate(a||r.title)+"</span>"}else s=t.icon.create(r.icon||n),s+='<span class="fr-sr-only">'+(t.language.translate(r.title)||"")+"</span>"
var l=r.popup?' data-popup="true"':"",u=r.modal?' data-modal="true"':"",c=t.shortcuts.get(n+".")
c=c?" ("+c+")":""
var p=n+"-"+t.id,d="dropdown-menu-"+p,h='<button id="'+p+'"type="button" tabIndex="-1" role="button"'+(r.toggle?' aria-pressed="false"':"")+("dropdown"==r.type||"options"==r.type?' aria-controls="'+d+'" aria-expanded="false" aria-haspopup="true"':"")+(r.disabled?' aria-disabled="true"':"")+' title="'+(t.language.translate(r.title)||"")+c+'" class="fr-command fr-btn'+("dropdown"==r.type||"options"==r.type?" fr-dropdown":"")+("options"==r.type?" fr-options":"")+" fr-btn-"+t.icon.getTemplate(r.icon)+(r.displaySelection?" fr-selection":"")+(r.back?" fr-back":"")+(r.disabled?" fr-disabled":"")+(i?"":" fr-hidden")+'" data-cmd="'+n+'"'+l+u+">"+s+"</button>"
if("dropdown"==r.type||"options"==r.type){var m='<div id="'+d+'" class="fr-dropdown-menu" role="listbox" aria-labelledby="'+p+'" aria-hidden="true"><div class="fr-dropdown-wrapper" role="presentation"><div class="fr-dropdown-content" role="presentation">'
